...
import httpx
import mariadb
SECRET_PATHS = ['/etc/secrets/tronity', '/etc/secrets/redis',
    '/run/secrets', './secrets']
SLEEPTIME = 1800
...
async def db_connect(my_logger):
    try:
        ...
        conn = mariadb.connect(user=load_secret('MARIADB_USERNAME'),
            password=load_secret('MARIADB_PASSWORD'), host=load_secret(
            'MARIADB_HOSTNAME'), port=3306, database=load_secret(
            'MARIADB_DATABASE'))
